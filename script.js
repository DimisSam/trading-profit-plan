...
function openForm2() {
    ...
    document.getElementById("kratiseis_display").innerText =
        document.getElementById("kratiseis").value;
}
...
function run10() {
    ...
    for (let i = 1; i <= 10; i++) {
        ...
        // Κράτηση (ίδιο ποσοστό με τη 1η φόρμα)
        let fee = w * kratiseisPct / 100;
        document.getElementById("fee" + i).innerText = fee.toFixed(2);

        // Ποσό ανάληψης μετά την κράτηση
        let wf = w - fee;
        document.getElementById("wf" + i).innerText = wf.toFixed(2);

        // Ανάληψη σε EUR (ίδιος τύπος με τη 1η φόρμα, χωρίς επιπλέον συντελεστή)
        let net = wf * usdcRate;
